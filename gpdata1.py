...
class GetData:
    def __init__(self, gpdm, gpmc, startdate, enddate, tdsdate='20230101', tdedate='20230204'):
    ...
    def get_gp_y(self):  # y不涉及到取新的来预测的问题
        ...
        for row in ysdata.itertuples([['日期', '收盘', '涨跌幅']]):
            ...
            if stock_zh_a_hist_df_shape[0] > 0 \
                    and row.日期 == ysdata.iloc[stock_zh_a_hist_df_shape[0] - 1, 0]:
    ...
    def get_gp_x(self, xlorcs='train'):
        bjlist_x = self.load_gp_xlsdata()
        if xlorcs == 'predict':
            bjlist_x = self.get_gp_akdata(xlorcs)
